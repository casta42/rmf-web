  ...
  paths: {
    ...
    '/alerts': {
      get: {
        tags: ['Alerts'],
        summary: 'Get Alerts',
        description: 'FR-31: filterable, paginated alert list (newest first by default).',
        ...
        parameters: [
          {
            ...
            schema: {
              ...
              description: "'open' (unresolved), 'resolved' (archive), or omit for all",
              title: 'Status',
            },
            description: "'open' (unresolved), 'resolved' (archive), or omit for all",
          },
          ...
          },
          {
            ...
            in: 'query',
            required: false,
            schema: { anyOf: [{ type: 'string' }, { type: 'null' }], title: 'Severity' },
          },
          {
            name: 'fleet',
            in: 'query',
            required: false,
            schema: { anyOf: [{ type: 'string' }, { type: 'null' }], title: 'Fleet' },
          },
          {
            name: 'robot',
            in: 'query',
            required: false,
            schema: { anyOf: [{ type: 'string' }, { type: 'null' }], title: 'Robot' },
          },
          {
            name: 'limit',
            in: 'query',
    ...
    '/alerts/{alert_id}/resolve': {
      post: {
        ...
        description: 'FR-31: operator resolution — archives the alert (never deletes).',
    ...
    '/zones': {
      get: {
        ...
        operationId: 'get_zones_zones_get',
        responses: {
          '200': {
            description: 'Successful Response',
            content: {
              'application/json': {
                schema: { type: 'object', title: 'Response Get Zones Zones Get' },
              },
            },
          },
        },
      ...
      tortoise__contrib__pydantic__creator__api_server__models__tortoise_models__alerts__Alert__leaf:
        {
          ...
          additionalProperties: false,
          type: 'object',
          required: ['id', 'original_id', 'category', 'message', 'unix_millis_created_time'],
          title: 'Alert',
          description:
            'General alert that can be triggered by events.<br/><br/>v2 (FR-31, GentleFleet fork): single-row lifecycle — an alert is<br/>created open, acknowledged in place, and RESOLVED in place (archived),<br/>never deleted. "Open" means `unix_millis_resolved_time` is null.<br/>Structured context fields (severity/fleet/robot/message) replace<br/>parsing the id string; the id format is unchanged for compatibility<br/>with the F-29/F-39 prefix sweeps.',
        },
